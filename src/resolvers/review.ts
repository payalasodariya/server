...
export class ReviewResolver {
  ...
  async updateReview(
    ...
    const result = await getConnection()
      ...
      .execute();
    if (!result.raw[0]) {
      throw new GraphQLError('You are not allowed to update this review')
    }
    return result.raw[0];
  }
  ...
  async deleteReview(
    ...
    const deletedResult = await Review.delete({ id, user_id: req.body.user_id });
    if (!deletedResult.affected) throw new GraphQLError('You are not allowed to delete this review')
    return true;
  }
}
